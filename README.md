...
- **项目宪法**：基于项目技术栈，派发 subagent 调研各厂商最佳实践，产出分层宪法体系——根 AGENTS.md 只做定位层（一层目录地图 + 子项目宪法索引），各技术子项目目录各生成 AGENTS.md 承载宪法深度，只存工程原则与约束
...
| `constitution-generator`（项目宪法生成器） | 根据项目技术栈生成分层项目宪法体系：按技术栈组件族划分调研主题，派发 subagent 调研各技术栈官方文档与厂商最佳实践，提炼为「只存工程原则与约束」的条款（业务数据契约归 `specs/` 职责行、变更记录外置 `CHANGELOG.md`）。根 AGENTS.md 只做定位层（项目定位 + 一层目录地图 + 强制阅读路由 + 子项目宪法索引 + 跨切总则），各技术子项目目录各生成 AGENTS.md 承载宪法深度，每份 AGENTS.md 同目录配套生成 CLAUDE.md（md 链接索引） | `<目标项目根>/AGENTS.md`（定位层）+ `<子项目>/AGENTS.md` ×N + 配套 `CLAUDE.md` ×(N+1) + `/docs/agmds-research/{yyyy-MM-dd}-{主题}.md` 调研报告；整仓单应用时仅 `<根>/AGENTS.md` + `<根>/CLAUDE.md` |
...
└── references/
    ├── template.md           # 定位层与子宪法必含章节骨架与逐章填写规则
    └── example.md            # 书面风格示例（根定位层 + 子项目宪法）
